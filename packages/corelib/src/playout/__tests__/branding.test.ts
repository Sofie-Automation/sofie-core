...
describe('branding', () => {
	...
	describe('resolvePieceForBranding', () => {
		...
		it('ignores overrides to properties which are not brandable', () => {
			const piece = {
				name: 'Piece 0',
				...
				branding: {
					// Blueprints are untyped JS, so these can be authored even though the types forbid them
					branding0: {
						name: 'Branded Piece 0',
			...
			expect(resolved?.name).toBe('Branded Piece 0')
			// A Branding cannot move a Piece between layers, as playout groups by them
			expect(resolved?.sourceLayerId).toBe('layer0')

...
function calculateExpectedDurationWithTransition(rawDuration: number, timings: PartCalculatedTimings): number {
	// toPartDelay, fromPartPostroll, and fromPartKeepalive need to be subtracted, because they are added to `fromPartRemaining` when the `fromPartRemaining` value is calculated.
	return Math.max(
		0,
		rawDuration -

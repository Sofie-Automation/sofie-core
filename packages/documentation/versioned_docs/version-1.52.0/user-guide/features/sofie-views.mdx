...
Another mode available to display a Segment is the List View. In this mode, each _Part_ and its contents are being displayed as a mini-timeline and its width is normalized to fit the screen, unless it's shorter than 30 seconds, in which case it will be scaled down accordingly.

![List View Mode](/img/docs/main/list_view.png)
...
The Switchboard allows the producer to turn automation _On_ and _Off_ for sets of devices, as well as re-route automation control between devices - both with an active rundown and when no rundown is active in a [Studio](../concepts-and-architecture.md#system-organization-studio--show-style).
...
![Camera Position View](/img/docs/main/features/camera-view.jpg)

A fullscreen view designed specifically for use on mobile devices or extra screens displaying a summary of the currently active Rundown, filtered for Parts containing Pieces matching particular Source Layers and Studio Labels.
